...
### モデルの準備（重要）
本プロジェクトでは、モデルバイナリ（`models/`）は Git 管理に含まれません。
以下のいずれかの方法でモデルを準備してください。
...
1. **開発・学習環境でビルドする**:
   ...
   make build-models
   ```
   ※ このコマンドにより、データ生成・学習・ONNXエクスポートが一括実行され、`models/` ディレクトリが構築されます。

2. **事前構築済みモデルをダウンロードする**:
   （社内ストレージやリリースセクションから `models.zip` を展開してプロジェクトルートに配置してください）
